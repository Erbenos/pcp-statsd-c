    ...
    for (i = 0; i < count; i++) {
            ...
            buffer[i] == '\0')
        {
            ...
            if (buffer[i] == ':' && previous_delimiter == ' ') {
                ...
                memcpy((*datagram)->name, attr, current_segment_length + 1);
                previous_delimiter = ':';
                ...
                char* startptr;
                char* endptr = attr + current_segment_length;
                if (attr[0] == '+') {
                ...
                if (!sanitize_type_val_string(attr, &type)) {
                    goto error_clean_up;
                }
                ...
                if (previous_delimiter == '@') {
                    ...
                    char* endptr = attr + current_segment_length;
                    double value = strtod(attr, &endptr);
                    if (attr == endptr || errno == ERANGE) {
                        goto error_clean_up;
                    }
                    (*datagram)->sampling = value;
...
main() {
    ...
    CHECK_ERROR("", NULL, NULL, 0, METRIC_TYPE_NONE, 0);
    CHECK_ERROR("wow", NULL, NULL, 0, METRIC_TYPE_NONE, 0);
    ...
    CHECK_ERROR("example:1.2|c", "example", NULL, 1.2, METRIC_TYPE_COUNTER, 0);
    CHECK_ERROR("example:1.000000004|c", "example", NULL, 1.000000004, METRIC_TYPE_COUNTER, 0);
    CHECK_ERROR("example:1.00000000000000000000000000000004|c", "example", NULL, 1.00000000000000000000000000000004, METRIC_TYPE_COUNTER, 0);
    SUITE_HEADER("Instance descriptors")
    CHECK_ERROR("example,instance=20:20|c", "example", "{\"instance\":\"20\"}", 20, METRIC_TYPE_COUNTER, 0);
    SUITE_HEADER("Tag descriptors")
    CHECK_ERROR("example,tagY=20,tagX=10:10|c", "example", "{\"tagX\":\"10\",\"tagY\":\"20\"}", 10, METRIC_TYPE_COUNTER, 0);
    SUITE_HEADER("Malformed tags")
    CHECK_ERROR("example,tags=dwq=qwddqwd=qwd:10|c", NULL, NULL, 0, METRIC_TYPE_NONE, 0);
    ...
    CHECK_ERROR("example:20|c@0.5", "example", NULL, 20, METRIC_TYPE_COUNTER, 0.5);
    CHECK_ERROR("example:20|c@15", "example", NULL, 20, METRIC_TYPE_COUNTER, 15);
    END_TEST();
}
